...
class ViewSurvivalMode {
   ...
   render(audio) {
      ...
      this.rules.innerHTML = "Space - shot, Ctrl - mines";
      ...
      this.audioRiffle.setAttribute("src", "audio/kalash1.mp3");
   ...
   updateEntities(dt) {
      ...
      // //--- обновление анимации зомби укусившего игрока ---
      // for (let i = 0; i < that.zombies1Win.length; i++) {
      //    that.zombies1Win[i].sprite.update(dt);
      // }

      // //--- обновление анимации убитого игрока ---
      // for (let i = 0; i < that.playerDead.length; i++) {
      //    that.playerDead[i].sprite.update(dt);
      // }
   ...
   checkCollisions() {
      ...
      //--- обнаружение столкновений зомби и пуль, зомби и мин, зомби и игрока ---
      checkCollisionsBullets(that.zombies1);
      checkCollisionsBullets(that.zombies2);
      checkCollisionsBullets(that.zombies3);
      checkCollisionsBullets(that.zombies4);

      checkCollisionsMines(that.zombies1);
      checkCollisionsMines(that.zombies2);
      checkCollisionsMines(that.zombies3);
      checkCollisionsMines(that.zombies4);      

      function checkCollisionsBullets(zombs) {
         for (let i = 0; i < zombs.length; i++) {
            ...
            if (that.boxCollides( [pos[0] + size[0]/4, pos[1] + size[1]/4], 
               ...
               // that.zombies1Win.push({
               //    pos: pos,
               //    sprite: new Sprite("images/zombie1_male_attack.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5, 6, 7, 8], null, true, Math.PI/2)
               // });

               // that.playerDead.push({
               //    pos: that.player.pos,
               //    sprite: new Sprite("images/man_gun_dead.png", [0, 0], [256, 256], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
               // });               
            }
         }
      }

      function checkCollisionsMines(zombs) {
         for (let i = 0; i < zombs.length; i++) {
            let pos = zombs[i].pos;
            let size = zombs[i].sprite.size;
   ...
   renderGame() {
      ...
      // that.renderEntities(that.zombies1Win);
      // that.renderEntities(that.playerDead);
   ...
   choicedPlayer(plr, weapon) {
      ...
      if (weapon === "riffle") {
         that.intervalBullet = 300;
         that.bulletSpeed = 1400;
         that.numberKillZomby = 1;
      ...
      if (weapon === "firethrower") {
         that.intervalBullet = 1800;
         that.bulletSpeed = 2000;
   ...
   play() {
      let imgs = [
         ...
         "images/zombie1_male_attack.png",
