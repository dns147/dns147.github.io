...
class ViewMap1 {
   constructor(container, modal) {
      ...
      this.zombies1Dead = [];
      this.zombies1Killed = [];
      this.zombies1Door2Dead = [];
      this.zombies1Door2Killed = [];
      this.zombies1Door2AddDead = [];
      this.zombies1Door2AddKilled = [];
      this.zombies1AfterDoor2Dead = [];
      this.zombies1AfterDoor2Killed = [];
      this.zombies1Door3Dead = [];
      this.zombies1Door3Killed = [];
      this.zombies1AfterDoor3Dead = [];
      this.zombies1AfterDoor3Killed = [];
      this.zombies1Door4Dead = [];
      this.zombies1Door4Killed = [];
      this.zombies1AfterDoor4Dead = [];
      this.zombies1AfterDoor4Killed = [];
      this.zombies1Door5Dead = [];
      this.zombies1Door5Killed = [];
      this.zombies1AfterDoor5Dead = [];
      this.zombies1AfterDoor5Killed = [];
      this.zombies1AfterDoor5AddDead = [];
      this.zombies1AfterDoor5AddKilled = [];
      this.zombies1Door6Dead = [];
      this.zombies1Door6Killed = [];
      this.zombies1Door6AddDead = [];
      this.zombies1Door6AddKilled = [];
   ...
   render(audio) {
      ...
      this.rules.innerHTML = "Space - shot";
      this.modalContainer.appendChild(this.rules);
      ...
      this.audioZombDead1 = document.createElement("audio");
      this.audioZombDead1.setAttribute("src", "audio/zombie_dead1.mp3");
      ...
      this.audioBlood = document.createElement("audio");
      this.audioBlood.setAttribute("src", "audio/blood.mp3");
   ...
   playGame() {
      ...
      let imgs = [
         ...
         "images/zombie1_male_dead.png",
         "images/zombie2_male_walk.png",
         "images/zombie2_male_dead.png",
         "images/zombie3_male_walk.png",
         "images/zombie3_male_dead.png",
         "images/zombie4_male_walk.png",
         "images/zombie4_male_dead.png",
   ...
   updateEntities(dt) {
      ...
      //--- обновление убиваемых зомби zombies1 ---
      for (let i = 0; i < that.zombies1Dead.length; i++) {
         that.zombies1Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Dead[i].sprite.done) {
            let pos = that.zombies1Dead[i].pos;

            that.zombies1Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, Math.PI/2)
            });
         }
      }

      //--- обновление убиваемых зомби zombies1Door2 ---
      for (let i = 0; i < that.zombies1Door2Dead.length; i++) {
         that.zombies1Door2Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Door2Dead[i].sprite.done) {
            let pos = that.zombies1Door2Dead[i].pos;

            that.zombies1Door2Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Door2Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, -Math.PI/2)
            });
         }
      ...
      //--- обновление убиваемых зомби zombies1Door2Add ---
      for (let i = 0; i < that.zombies1Door2AddDead.length; i++) {
         that.zombies1Door2AddDead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Door2AddDead[i].sprite.done) {
            let pos = that.zombies1Door2AddDead[i].pos;

            that.zombies1Door2AddDead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Door2AddKilled.push({
               pos: pos,
               sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, 0)
            });
         }
      }

      //--- обновление убиваемых зомби zombies1AfterDoor2 ---
      for (let i = 0; i < that.zombies1AfterDoor2Dead.length; i++) {
         that.zombies1AfterDoor2Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1AfterDoor2Dead[i].sprite.done) {
            let pos = that.zombies1AfterDoor2Dead[i].pos;

            that.zombies1AfterDoor2Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1AfterDoor2Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, -Math.PI/2)
            });
         }
      }

      //--- обновление убиваемых зомби zombies1Door3 ---
      for (let i = 0; i < that.zombies1Door3Dead.length; i++) {
         that.zombies1Door3Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Door3Dead[i].sprite.done) {
            let pos = that.zombies1Door3Dead[i].pos;

            that.zombies1Door3Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Door3Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, Math.PI)
            });
         }
      }

      //--- обновление убиваемых зомби zombies1AfterDoor3 ---
      for (let i = 0; i < that.zombies1AfterDoor3Dead.length; i++) {
         that.zombies1AfterDoor3Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1AfterDoor3Dead[i].sprite.done) {
            let pos = that.zombies1AfterDoor3Dead[i].pos;

            that.zombies1AfterDoor3Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1AfterDoor3Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, -Math.PI/2)
            });
         }
      ...
      //--- обновление убиваемых зомби zombies1Door4 ---
      for (let i = 0; i < that.zombies1Door4Dead.length; i++) {
         that.zombies1Door4Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Door4Dead[i].sprite.done) {
            let pos = that.zombies1Door4Dead[i].pos;

            that.zombies1Door4Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Door4Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, -Math.PI/2)
            });
         }
      ...
      //--- обновление убиваемых зомби zombies1AfterDoor4 ---
      for (let i = 0; i < that.zombies1AfterDoor4Dead.length; i++) {
         that.zombies1AfterDoor4Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1AfterDoor4Dead[i].sprite.done) {
            let pos = that.zombies1AfterDoor4Dead[i].pos;

            that.zombies1AfterDoor4Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1AfterDoor4Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, 0)
            });
         }
      }

      //--- обновление убиваемых зомби zombies1Door5 ---
      for (let i = 0; i < that.zombies1Door5Dead.length; i++) {
         that.zombies1Door5Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Door5Dead[i].sprite.done) {
            let pos = that.zombies1Door5Dead[i].pos;

            that.zombies1Door5Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Door5Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, 0)
            });
         }
      ...
      //--- обновление убиваемых зомби zombies1AfterDoor5 ---
      for (let i = 0; i < that.zombies1AfterDoor5Dead.length; i++) {
         that.zombies1AfterDoor5Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1AfterDoor5Dead[i].sprite.done) {
            let pos = that.zombies1AfterDoor5Dead[i].pos;

            that.zombies1AfterDoor5Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1AfterDoor5Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie3_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, 0)
            });
         }
      }

      //--- обновление убиваемых зомби zombies1AfterDoor5Add ---
      for (let i = 0; i < that.zombies1AfterDoor5AddDead.length; i++) {
         that.zombies1AfterDoor5AddDead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1AfterDoor5AddDead[i].sprite.done) {
            let pos = that.zombies1AfterDoor5AddDead[i].pos;

            that.zombies1AfterDoor5AddDead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1AfterDoor5AddKilled.push({
               pos: pos,
               sprite: new Sprite("images/zombie4_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, 0)
            });
         }
      }

      //--- обновление убиваемых зомби zombies1Door6 ---
      for (let i = 0; i < that.zombies1Door6Dead.length; i++) {
         that.zombies1Door6Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Door6Dead[i].sprite.done) {
            let pos = that.zombies1Door6Dead[i].pos;

            that.zombies1Door6Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Door6Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie3_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, 0)
            });
         }
      ...
      //--- обновление убиваемых зомби zombies1Door6Add ---
      for (let i = 0; i < that.zombies1Door6AddDead.length; i++) {
         that.zombies1Door6AddDead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies1Door6AddDead[i].sprite.done) {
            let pos = that.zombies1Door6AddDead[i].pos;

            that.zombies1Door6AddDead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies1Door6AddKilled.push({
               pos: pos,
               sprite: new Sprite("images/zombie4_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, Math.PI/2)
            });
         }
      }

      //--- обновление убиваемых зомби zombies2 ---
      for (let i = 0; i < that.zombies2Dead.length; i++) {
         that.zombies2Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies2Dead[i].sprite.done) {
            let pos = that.zombies2Dead[i].pos;

            that.zombies2Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies2Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, -Math.PI/2)
            });
         }
      ...
      //--- обновление убиваемых зомби zombies3 ---
      for (let i = 0; i < that.zombies3Dead.length; i++) {
         that.zombies3Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies3Dead[i].sprite.done) {
            let pos = that.zombies3Dead[i].pos;

            that.zombies3Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies3Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, 0)
            });
         }
      ...
      //--- обновление убиваемых зомби zombies4 ---
      for (let i = 0; i < that.zombies4Dead.length; i++) {
         that.zombies4Dead[i].sprite.update(dt);
         
         //--- удаление убиваемых зомби, если анимация завершена ---
         if (that.zombies4Dead[i].sprite.done) {
            let pos = that.zombies4Dead[i].pos;

            that.zombies4Dead.splice(i, 1);
            i--;

            //--- добавление убитых зомби ---
            that.zombies4Killed.push({
               pos: pos,
               sprite: new Sprite("images/zombie4_male_dead.png", [0, 0], [410, 410], 10, [5], null, false, Math.PI)
            });
         }
      }

      //--- обновление убитых зомби ---
      for (let i = 0; i < that.zombies1Killed.length; i++) {
         that.zombies1Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1Door2Killed.length; i++) {
         that.zombies1Door2Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1Door2AddKilled.length; i++) {
         that.zombies1Door2AddKilled[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1AfterDoor2Killed.length; i++) {
         that.zombies1AfterDoor2Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1Door3Killed.length; i++) {
         that.zombies1Door3Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1AfterDoor3Killed.length; i++) {
         that.zombies1AfterDoor3Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1Door4Killed.length; i++) {
         that.zombies1Door4Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1AfterDoor4Killed.length; i++) {
         that.zombies1AfterDoor4Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1Door5Killed.length; i++) {
         that.zombies1Door5Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1AfterDoor5Killed.length; i++) {
         that.zombies1AfterDoor5Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1AfterDoor5AddKilled.length; i++) {
         that.zombies1AfterDoor5AddKilled[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1Door6Killed.length; i++) {
         that.zombies1Door6Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies1Door6AddKilled.length; i++) {
         that.zombies1Door6AddKilled[i].sprite.update(dt);
      }
      
      for (let i = 0; i < that.zombies2Killed.length; i++) {
         that.zombies2Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies3Killed.length; i++) {
         that.zombies3Killed[i].sprite.update(dt);
      }

      for (let i = 0; i < that.zombies4Killed.length; i++) {
         that.zombies4Killed[i].sprite.update(dt);
      }
   ...
   checkCollisions() {
      ...
      function checkCollisionsDoor(doors) {
         ...
         for (let j = 0; j < that.bullets.length; j++) {
            ...
            if (that.boxCollides( [pos1[0] - size1[0]/2, pos1[1] - size1[1]/2], 
                                  size1, posBall, sizeBall )) {
      ...
      function checkCollisionsAll(zombs) {
         for (let i = 0; i < zombs.length; i++) {
            ...
            for (let j = 0; j < that.bullets.length; j++) {
               ...
               if (that.boxCollides( [pos[0] - size[0]/2, pos[1] - size[1]/2], 
                                     size, posBall, sizeBall )) {
                  that.audioZombDead1.play();
                  that.audioBlood.play();

                  //--- Удаление зомби из массива при его убийстве ---
                  zombs.splice(i, that.numberKillZomby);
                  i--;

                  //--- добавление убиваемых зомби
                  //--- идущие справа налево ---
                  switch(zombs) {
                     case that.zombies1:
                        that.zombies1Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, Math.PI/2)
                        });
                        break;

                     case that.zombies1Door6Add:
                        that.zombies1Door6AddDead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie4_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, Math.PI/2)
                        });
                        break;

                     //--- идущие слева направо ---
                     case that.zombies1Door2:
                        that.zombies1Door2Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, -Math.PI/2)
                        });
                        break;

                     case that.zombies1AfterDoor2:
                        that.zombies1AfterDoor2Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, -Math.PI/2)
                        });
                        break;                        

                     case that.zombies2:
                        that.zombies2Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, -Math.PI/2)
                        });
                        break;

                     case that.zombies1AfterDoor3:
                        that.zombies1AfterDoor3Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, -Math.PI/2)
                        });
                        break;

                     case that.zombies1AfterDoor3Add:
                        that.zombies1AfterDoor3Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, -Math.PI/2)
                        });
                     ...
                     case that.zombies1Door4:
                        that.zombies1Door4Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, -Math.PI/2)
                        });
                        break;
                     ...
                     //--- идущие сверху вниз ---
                     case that.zombies3:
                        that.zombies3Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
                        });
                        break;

                     case that.zombies1Door2Add:
                        that.zombies1Door2AddDead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
                        });
                        break;

                     case that.zombies1AfterDoor4:
                        that.zombies1AfterDoor4Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie1_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
                        });
                        break;

                     case that.zombies1Door5:
                        that.zombies1Door5Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
                        });
                        break;

                     case that.zombies1AfterDoor5:
                        that.zombies1AfterDoor5Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie3_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
                        });
                        break;

                     case that.zombies1AfterDoor5Add:
                        that.zombies1AfterDoor5AddDead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie4_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
                        });
                        break;

                     case that.zombies1Door6:
                        that.zombies1Door6Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie3_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, 0)
                        });
                        break;
                     ...
                     //--- идущие снизу вверх ---
                     case that.zombies4:
                        that.zombies4Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie4_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, Math.PI)
                        });
                        break;

                     case that.zombies1Door3:
                        that.zombies1Door3Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, Math.PI)
                        });
                        break;

                     case that.zombies1Door3Add:
                        that.zombies1Door3Dead.push({
                           pos: pos,
                           sprite: new Sprite("images/zombie2_male_dead.png", [0, 0], [410, 410], 20, [0, 1, 2, 3, 4, 5], null, true, Math.PI)
                        });
            ...
            //--- обнаружение столкновений зомби и игрока ---
            if (that.boxCollides( [pos[0] + size[0]/4, pos[1] + size[1]/4], 
                                  [size[0]/2, size[1]/2], 
                                  that.player.pos, that.player.sprite.size )) {
               that.gameOver();
            }
         }
      }
   ...
   makeZombiesDoor1() {
      ...
      that.timerId1Add = setTimeout(stopZombies, 14000);
   ...
   renderGame() {
      ...
      that.renderEntities(that.zombies1Door2Killed);
      that.renderEntities(that.zombies1Door2AddKilled);
      that.renderEntities(that.zombies1AfterDoor2Killed);
      that.renderEntities(that.zombies1Door3Killed);
      that.renderEntities(that.zombies1AfterDoor3Killed);
      that.renderEntities(that.zombies1Door4Killed);
      that.renderEntities(that.zombies1AfterDoor4Killed);
      that.renderEntities(that.zombies1Door5Killed);
      that.renderEntities(that.zombies1AfterDoor5Killed);
      that.renderEntities(that.zombies1AfterDoor5AddKilled);
      that.renderEntities(that.zombies1Door6Killed);
      that.renderEntities(that.zombies1Door6AddKilled);
      ...
      that.renderEntities(that.zombies1Dead);
      that.renderEntities(that.zombies1Door2Dead);
      that.renderEntities(that.zombies1Door2AddDead);
      that.renderEntities(that.zombies1AfterDoor2Dead);
      that.renderEntities(that.zombies1Door3Dead);
      that.renderEntities(that.zombies1AfterDoor3Dead);
      that.renderEntities(that.zombies1Door4Dead);
      that.renderEntities(that.zombies1AfterDoor4Dead);
      that.renderEntities(that.zombies1Door5Dead);
      that.renderEntities(that.zombies1AfterDoor5Dead);
      that.renderEntities(that.zombies1AfterDoor5AddDead);
      that.renderEntities(that.zombies1Door6Dead);
      that.renderEntities(that.zombies1Door6AddDead);      
      
      that.renderEntities(that.zombies2Dead);    
      that.renderEntities(that.zombies3Dead);
      that.renderEntities(that.zombies4Dead);
